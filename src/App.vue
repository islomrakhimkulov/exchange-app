  ...
  <div id="app">
    ...
    <div class="container">
      ...
      <div class="container-four">
        <h4 id="lastlyUpdated">Last Updated: {{ data.time_last_update_utc }}</h4>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    calculateResults() {
      fetch(
        `https://v6.exchangerate-api.com/v6/${"576a80a05f5c133ddf99812d"}/latest/${
          this.currency_one
        }`
      )
        .then((res) => res.json())
...
.container-two button {
  padding: 5px;
  font-size: 18px;
  background: linear-gradient(#5fbaa7,#41B883);
  color: #fff;
  width: 20%;
  margin: auto;
  height: 10%;
